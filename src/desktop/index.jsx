...
(PLUGIN_ID => {
  ...
  kintone.events.on(['mobile.app.record.create.show'], async (event) => {
    ...
    if (data.check) {
      $('#appForm-gaia').remove()
      render(<FormRegister event={event} isAdmin={data.isAdmin}  isMobile={true}/>,
  ...
  kintone.events.on(['mobile.app.record.edit.show'], async (event) => {
    ...
    if (data.check) {
      $('#appForm-gaia').remove()
      render( <FormRegister event={event} isAdmin={data.isAdmin} type={'edit'}  isMobile={true}/>,
        document.getElementsByClassName('gaia-mobile-v2-ui-tabview')[0]);
    } else {
      logout();
    }
  ...
  kintone.events.on(['mobile.app.record.detail.show'], async (event) => {
    let data = await routerPage();
    if (data.check) {
      render(<Detail  isAdmin={data.isAdmin} record={event.record} isMobile={true} />,
        document.getElementById('record-gaia'));
    } else {
      logout();
    }
  });
